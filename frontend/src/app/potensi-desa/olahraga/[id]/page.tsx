...
export default function OlahragaDetailPage() {
      ...
      <>
        ...
        <main className="min-h-screen flex items-center justify-center">
          <div className="text-center">
            <p className="text-gray-500 text-lg mb-4">Data olahraga tidak ditemukan.</p>
            <button
              onClick={() => router.back()}
  ...
  const prevSlide = () => setCurrentSlide((p) => (p === 0 ? cabang.galeri.length - 1 : p - 1))
  const nextSlide = () => setCurrentSlide((p) => (p === cabang.galeri.length - 1 ? 0 : p + 1))
        ...
        <section className="bg-gradient-to-br from-primary-500 to-accent-600 text-white py-12 px-4">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="flex items-center gap-6">
              <div className="text-7xl md:text-8xl drop-shadow-lg">{cabang.icon}</div>
              <div>
                <h1 className="text-3xl md:text-4xl font-bold mb-1">{cabang.nama}</h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="card flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-800">{cabang.jumlahAnggota}</p>
              </div>
            </div>
            <div className="card flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-800">{cabang.prestasi.length}</p>
              </div>
            </div>
            <div className="card flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-800">{cabang.jadwal.length}x/minggu</p>
              </div>
            </div>
          </div>
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="bg-primary-50 rounded-2xl p-4">
                <p className="text-xs text-gray-400 font-medium mb-1">Pelatih / Instruktur</p>
                <p className="font-bold text-gray-800">{cabang.pelatih}</p>
              </div>
              <div className="bg-accent-50 rounded-2xl p-4">
                <p className="text-xs text-gray-400 font-medium mb-1">Ketua</p>
                <p className="font-bold text-gray-800">{cabang.ketua}</p>
              </div>
            </div>
          </div>
          ...
          <div className="card">
            ...
            <div className="relative rounded-2xl overflow-hidden bg-gray-100" style={{ aspectRatio: '16/9' }}>
              ...
              <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-primary-100 to-accent-100">
                <div className="text-center">
                  <div className="text-6xl mb-2">{cabang.icon}</div>
                  <p className="text-gray-500 text-sm">Foto {cabang.nama} {currentSlide + 1}/{cabang.galeri.length}</p>
                  <p className="text-gray-400 text-xs mt-1">{cabang.galeri[currentSlide]}</p>
                </div>
              </div>
              {/* Uncomment ini saat foto sudah tersedia di /public/images/olahraga/ */}
              {/* <Image src={cabang.galeri[currentSlide]} alt={`Foto ${cabang.nama} ${currentSlide + 1}`} fill className="object-cover" /> */}
